...
const DragableBodyRow = ({index, moveRow, data}) => {
	const ref = useRef();
		...
		<tr
			...
			<td
				...
				<div className="flex justify-start items-center w-full">
					<span className="text-black-tertiary text-sm">
						{data.id}
					</span>
				</div>
			</td>
			<td className="border-b border-t border-diver" width="35%">
				<div className="flex justify-start items-center w-full py-4 space-x-4 pr-4">
					...
					<div className="text-white text-xl">{data.title}</div>
				</div>
			</td>
			<td className="border-b border-t border-diver" width="40%">
				<div className="flex justify-start items-center w-full">
					<span className="text-green">{data.username}</span>
				</div>
			</td>
			<td
				className="rounded-r-2xl border-b border-t border-diver border-r pr-4"
				width="20%">
				<div className="flex flex-row justify-end items-center w-full space-x-2">
					...
					<svg
						...
						<g clipPath="url(#clip0_2478_1113)">
							<path
...
const AdminDashboardPage = () => {
	...
	const moveRow = useCallback(
		(dragIndex, hoverIndex) => {
			...
			const items = Array.from(videoListRef.current);
			items.splice(hoverIndex, 0, items.splice(dragIndex, 1)[0]);
			videoListRef.current = [...items];
			setVideoList([...items]);
		},
	...
	return (
		<>
			<div className="w-full flex flex-col justify-start items-center h-screen text-gray-700 font-thin max-w-7xl mx-auto pt-10">
				<nav className="flex flex-row justify-between items-center w-full mb-28">
					<span className="uppercase text-white text-5xl font-black">
						app
					</span>
					<button
						className="flex justify-center items-center outline-none bg-green px-6 py-[14px] rounded-[40px]"
						onClick={() => {
							dispatch({
								type: 'LOGOUT',
							});
				...
				<main className="flex flex-col justify-start items-start w-full">
					<div className="flex flex-row justify-between items-center mb-8 w-full">
						<h4 className="text-white text-[40px]">
							Today's leaderboard
						</h4>
						<div className="flex flex-row justify-start items-center gap-4 bg-black-item-bg px-10 py-[18px] rounded-2xl">
							...
							<div className="bg-green px-[10px] py-1 rounded-lg">
								<span className="uppercase text-black text-base">
									submissions open
								</span>
							</div>
							<div>
						...
						<div className="w-full h-full overflow-y-auto">
							<table className="w-full table-fixed border-separate border-spacing-y-6">
								<thead className="w-full pb-2">
									<tr>
										<th className="text-black-tertiary whitespace-nowrap text-sm font-normal text-left w-[5%] pl-4">
											#
										</th>
										<th className="text-black-tertiary whitespace-nowrap text-sm font-normal text-left w-[35%]">
											Title
										</th>
										<th className="text-black-tertiary whitespace-nowrap text-sm font-normal text-left w-[40%]">
											Author
										</th>
